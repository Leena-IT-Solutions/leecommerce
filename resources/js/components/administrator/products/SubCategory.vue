...
<div class="crud-wrapper animate-fade-in">
        ...
        <div class="row align-items-center justify-content-between g-3">
            <div class="col-12 col-sm-auto">
                <div class="title-container">
                    <h5 class="page-title text-uppercase m-0">Sub Category Manager</h5>
                    <span v-if="totalSubCategories !== null" class="count-badge">
                        <i class="fas fa-folder-open mr-1"></i> {{ totalSubCategories }} Total
                    </span>
                </div>
            </div>
    ...
    <div v-if="isFormOpen" class="crud-card mb-4 animate-fade-in">
        <div class="card-header-sub mb-3">
            <h6 class="text-uppercase m-0 font-weight-bold text-slate-800">
                {{ id === 0 ? 'Create New Sub Category' : 'Edit Sub Category #' + id }}
            </h6>
        </div>
        ...
        <form @submit.prevent="saveSubCategory">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-sm-6 col-md-3">
                    ...
                    <select class="form-control form-select-custom" id="category_id" v-model="category_id" @change="clearFieldError('category_id')">
                ...
                <div class="col-12 col-md-6">
                    <label class="disable-select form-label" for="sub_category">Sub Category Name</label>
                    <input class="form-control" type="text" id="sub_category" v-model="sub_category" placeholder="Enter Sub Category" @input="clearFieldError('sub_category')">
                    <span v-if="validationErrors.sub_category" class="disable-select form-error">{{ validationErrors.sub_category }}</span>
                </div>

                <!-- Sub Category Media Upload -->
                <div class="col-12">
                    <label class="disable-select form-label">Sub Category Media Image</label>
                    <div class="d-flex align-items-center flex-wrap gap-3 mb-2">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <span v-else class="disable-select form-hint">Upload a sub-category thumbnail image.</span>
                        </div>
                    </div>
                </div>
    ...
    <data-search
        :columns="[
            { value: 'id', label: 'ID' },
            { value: 'sub_category', label: 'Sub Category Name' },
        ...
        <div class="table-card">
            <div class="table-responsive">
                <table class="table" id="subcategories-table">
                    ...
                    <tbody>
                        <tr v-for="sub in subcategories" :key="sub.id" class="table-row-hover">
                            <td class="text-center font-weight-bold text-slate-500 align-middle">#{{ sub.id }}</td>
                            <td class="align-middle font-medium text-slate-800">{{ sub.data_category }}</td>
                            <td class="align-middle text-slate-700">{{ sub.sub_category }}</td>
                            <td class="align-middle">
                                <span class="badge" :class="sub.display === 'Show' ? 'bg-success-light text-success-dark' : 'bg-slate-light text-slate-dark'">
                                    {{ sub.display }}
                                </span>
                            </td>
                            <td class="align-middle text-slate-600 text-truncate" style="max-width: 200px;">{{ sub.title }}</td>
                            <td class="text-center align-middle">
                                <div class="table-media-container" v-if="sub.media">
                                    <img :src="sub.media" class="table-cell-image" alt="SubCategory">
                                </div>
                                <span v-else class="text-slate-400 font-italic small">No image</span>
                            </td>
                            <td class="text-right align-middle text-nowrap">
                                <button @click="editSubCategory(sub)" class="btn btn-action-edit" title="Edit Sub Category"><i class="fas fa-pen"></i></button>
                                <button @click="deleteSubCategory(sub.id)" class="btn btn-action-delete" title="Delete Sub Category"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr v-if="subcategories.length === 0">
                            <td colspan="7" class="text-center py-5 text-slate-400">
                                <div class="empty-state">
                                    ...
                                    <p class="m-0 font-medium">No sub-categories found matching the query.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div class="footer-actions-panel mb-5">
        <div class="d-flex justify-content-between align-items-center flex-wrap g-3">
            <button class="btn btn-load-more text-uppercase" @click="loadMore" :disabled="currentPage == lastPage || subcategories.length === 0">
...
export default {
    ...
    methods: {
        ...
        fetchSubCategories(page = 1) {
            ...
            window.axios.get('/crud', { params })
                .then(res => {
                    ...
                    this.totalSubCategories = res.data.total;
        ...
        editSubCategory(sub) {
            ...
            window.scroll(0, 0);
            this.id = sub.id;
            this.display = sub.display;
            this.category_id = sub.category_id;
            this.sub_category = sub.sub_category;
            this.media = sub.media;
            this.title = sub.title;
            this.keywords = sub.keywords;
            this.description = sub.description;
            
            this.mediaFile = null;
            this.mediaFilename = sub.media ? sub.media.split('/').pop() : '';
            this.mediaPreview = '';
            this.validationErrors = {};
            this.successMessage = '';
            this.errorMessage = '';
        ...
        saveSubCategory() {
            ...
            let crudnames = ['display', 'category_id', 'sub_category', 'title', 'keywords', 'description'];
            let crudtypes = ['select', 'select', 'text', 'text', 'textarea', 'textarea'];
            let crudvalidation = ['required', 'required', 'required|string', 'required|string', 'required|string', 'required|string'];
            ...
            window.axios.post('/crud', fd)
                .then(res => {
                    this.successMessage = this.id === 0 ? 'Sub Category created successfully.' : 'Sub Category updated successfully.';
                    this.fetchSubCategories(1);
                ...
                .catch(error => {
                    if (error.response && error.response.status === 422) {
                        ...
                        this.errorMessage = 'An error occurred while saving the sub-category.';
        ...
        deleteSubCategory(id) {
            const c = confirm('Are you sure you want to delete this sub-category?');
            if (c) {
        ...
        exportToExcel() {
            const tableSelect = document.getElementById('subcategories-table');
            const tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
            const filename = 'subcategories_list.xls';
