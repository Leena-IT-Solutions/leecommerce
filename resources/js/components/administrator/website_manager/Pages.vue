...
<div class="crud-wrapper animate-fade-in">
        ...
        <div class="row align-items-center justify-content-between g-3">
            <div class="col-12 col-sm-auto">
                <div class="title-container">
                    <h5 class="page-title text-uppercase m-0">Pages Manager</h5>
                    <span v-if="totalPages !== null" class="count-badge">
                        <i class="fas fa-file-alt mr-1"></i> {{ totalPages }} Total Pages
                    </span>
                </div>
            </div>
    ...
    <div v-if="isFormOpen" class="crud-card mb-4 animate-fade-in">
        <div class="card-header-sub mb-3">
            <h6 class="text-uppercase m-0 font-weight-bold text-slate-800">
                {{ id === 0 ? 'Create New Page' : 'Edit Page #' + id }}
            </h6>
        </div>
        ...
        <form @submit.prevent="savePage">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    <label class="disable-select form-label" for="page">Page Name</label>
                    <input class="form-control" type="text" id="page" v-model="page" placeholder="Enter Page Name" @input="clearFieldError('page')">
                        ...
                        <div class="flex-grow-1">
                            <div class="custom-file-wrapper">
                                <div class="custom-file">
                                    ...
                                    <label class="custom-file-label text-truncate" for="media">{{ mediaFilename || 'Choose image...' }}</label>
                                </div>
                            </div>
                            <span v-if="validationErrors.media" class="disable-select form-error">{{ validationErrors.media }}</span>
                            <span v-else class="disable-select form-hint">Upload a promotional image for this page.</span>
                        </div>
                    </div>
                </div>
                ...
                <table class="table" id="pages-table">
                    <thead>
                        <tr>
                            ...
                            <th>Page Name</th>
                            <th>Page Title</th>
                            <th class="text-center" style="width: 150px;">Media Image</th>
                            <th style="width: 140px;" class="text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="pg in pages" :key="pg.id" class="table-row-hover">
                            <td class="text-center font-weight-bold text-slate-500 align-middle">#{{ pg.id }}</td>
                            <td class="align-middle font-medium text-slate-800">{{ pg.page }}</td>
                            <td class="align-middle text-slate-700">{{ pg.title }}</td>
                            <td class="text-center align-middle">
                                <div class="table-media-container" v-if="pg.media" style="width: 80px; height: 42px; border-radius: 4px;">
                                    <img :src="pg.media" class="table-cell-image" alt="Page Media">
                                </div>
                                <span v-else class="text-slate-400 font-italic small">No image</span>
                            </td>
                            <td class="text-right align-middle text-nowrap">
                                <button @click="editPage(pg)" class="btn btn-action-edit" title="Edit Page"><i class="fas fa-pen"></i></button>
                                <button @click="deletePage(pg.id)" class="btn btn-action-delete" title="Delete Page"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr v-if="pages.length === 0">
                            <td colspan="5" class="text-center py-5 text-slate-400">
                                <div class="empty-state">
                                    <i class="fas fa-file-alt fa-2x mb-3 text-slate-300"></i>
                                    <p class="m-0 font-medium">No pages found matching the query.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div class="footer-actions-panel mb-5">
        <div class="d-flex justify-content-between align-items-center flex-wrap g-3">
            <button class="btn btn-load-more text-uppercase" @click="loadMore" :disabled="currentPage == lastPage || pages.length === 0">
    ...
    methods: {
        ...
        loadMore() {
            if (this.currentPage < this.lastPage) {
                this.fetchPages(this.currentPage + 1);
            }
        },
        ...
        savePage() {
            ...
            window.axios.post('/crud', fd)
                .then(res => {
                    this.successMessage = this.id === 0 ? 'Page created successfully.' : 'Page updated successfully.';
                    this.fetchPages(1);
                ...
                .catch(error => {
                    if (error.response && error.response.status === 422) {
                        ...
                        this.errorMessage = 'An error occurred while saving the page.';
        ...
        deletePage(id) {
            const c = confirm('Are you sure you want to delete this page?');
            if (c) {
        ...
        exportToExcel() {
            const tableSelect = document.getElementById('pages-table');
            const tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
            const filename = 'pages_list.xls';
