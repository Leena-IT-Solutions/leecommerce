...
<div class="crud-wrapper animate-fade-in">
        ...
        <div class="row align-items-center justify-content-between g-3">
            <div class="col-12 col-sm-auto">
                <div class="title-container">
                    <h5 class="page-title text-uppercase m-0">Features Manager</h5>
                    <span v-if="totalFeatures !== null" class="count-badge">
                        <i class="fas fa-star mr-1"></i> {{ totalFeatures }} Total Features
                    </span>
                </div>
            </div>
    ...
    <div v-if="isFormOpen" class="crud-card mb-4 animate-fade-in">
        <div class="card-header-sub mb-3">
            <h6 class="text-uppercase m-0 font-weight-bold text-slate-800">
                {{ id === 0 ? 'Create New Feature' : 'Edit Feature #' + id }}
            </h6>
        </div>
        ...
        <form @submit.prevent="saveFeature">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-sm-6 col-md-8">
                    ...
                    <input class="form-control" type="text" id="title" v-model="title" placeholder="Enter Feature Title" @input="clearFieldError('title')">
                ...
                <!-- Media -->
                <div class="col-12">
                    <label class="disable-select form-label">Feature Image</label>
                        ...
                        <div class="flex-grow-1">
                            <div class="custom-file-wrapper">
                                <div class="custom-file">
                                    ...
                                    <label class="custom-file-label text-truncate" for="media">{{ mediaFilename || 'Choose feature image...' }}</label>
                                </div>
                            </div>
                            <span v-if="validationErrors.media" class="disable-select form-error">{{ validationErrors.media }}</span>
                            <span v-else class="disable-select form-hint">Upload a promotional feature image / banner.</span>
                        </div>
                    </div>
                </div>
                ...
                <table class="table" id="features-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="width: 150px;">Media Banner</th>
                            <th style="width: 120px;">Display</th>
                            <th style="width: 140px;" class="text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="feat in features" :key="feat.id" class="table-row-hover">
                            <td class="text-center font-weight-bold text-slate-500 align-middle">#{{ feat.id }}</td>
                            <td class="align-middle font-medium text-slate-800">{{ feat.title }}</td>
                            <td class="align-middle text-slate-600 font-monospace small" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                <a :href="feat.link" target="_blank" title="Open Link">{{ feat.link }}</a>
                            </td>
                            <td class="text-center align-middle">
                                <div class="table-media-container" v-if="feat.media" style="width: 80px; height: 42px; border-radius: 4px;">
                                    <img :src="feat.media" class="table-cell-image" alt="Feature Image">
                                </div>
                                <span v-else class="text-slate-400 font-italic small">No image</span>
                            </td>
                            <td class="align-middle">
                                <span class="badge" :class="feat.display === 'Show' ? 'bg-success-light text-success-dark' : 'bg-slate-light text-slate-dark'">
                                    {{ feat.display }}
                                </span>
                            </td>
                            <td class="text-right align-middle text-nowrap">
                                <button @click="editFeature(feat)" class="btn btn-action-edit" title="Edit Feature"><i class="fas fa-pen"></i></button>
                                <button @click="deleteFeature(feat.id)" class="btn btn-action-delete" title="Delete Feature"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr v-if="features.length === 0">
                            <td colspan="6" class="text-center py-5 text-slate-400">
                                <div class="empty-state">
                                    <i class="fas fa-star fa-2x mb-3 text-slate-300"></i>
                                    <p class="m-0 font-medium">No features found matching the query.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
    ...
    <div class="footer-actions-panel mb-5">
        <div class="d-flex justify-content-between align-items-center flex-wrap g-3">
            <button class="btn btn-load-more text-uppercase" @click="loadMore" :disabled="currentPage == lastPage || features.length === 0">
    ...
    methods: {
        ...
        fetchFeatures(page = 1) {
            ...
            window.axios.get('/crud', { params })
                .then(res => {
                    ...
                    this.totalFeatures = res.data.total;
                    
                    res.data.data.forEach(feat => {
        ...
        loadMore() {
            if (this.currentPage < this.lastPage) {
                this.fetchFeatures(this.currentPage + 1);
            }
        },

        submitSearch() {
            this.fetchFeatures(1);
        },
        ...
        editFeature(feat) {
            ...
            window.scroll(0, 0);
            this.id = feat.id;
            this.display = feat.display;
            this.title = feat.title;
            this.link = feat.link;
            this.media = feat.media;
            
            this.mediaFile = null;
            this.mediaFilename = feat.media ? feat.media.split('/').pop() : '';
            this.mediaPreview = '';
            this.validationErrors = {};
            this.successMessage = '';
            this.errorMessage = '';
        ...
        saveFeature() {
            ...
            window.axios.post('/crud', fd)
                .then(res => {
                    this.successMessage = this.id === 0 ? 'Feature created successfully.' : 'Feature updated successfully.';
                    this.fetchFeatures(1);
                ...
                .catch(error => {
                    if (error.response && error.response.status === 422) {
                        ...
                        this.errorMessage = 'An error occurred while saving the feature.';
        ...
        deleteFeature(id) {
            const c = confirm('Are you sure you want to delete this feature?');
            if (c) {
        ...
        exportToExcel() {
            const tableSelect = document.getElementById('features-table');
            const tableHTML = tableSelect.outerHTML.replace(/ /g, '%20');
            const filename = 'features_list.xls';
